...
export class EditEntryPage {
  ...
  async updateSQL(rowid) {
    if (this.data.name != '') {
      this._db
        .executeSql('UPDATE myfamily SET name=? WHERE rowid=?', [
          this.data.name,
          rowid,
        ])
        .then((res) => {
    ...
    if (this.data.birthday != '') {
      this._db
        .executeSql('UPDATE myfamily SET birthday=? WHERE rowid=?', [
          this.data.birthday,
          rowid,
        ])
        .then((res) => {
    ...
    if (this.data.relation != '') {
      this._db
        .executeSql('UPDATE myfamily SET relation=? WHERE rowid=?', [
          this.data.relation,
          rowid,
        ])
        .then((res) => {
    ...
    if (this.data.email != '') {
      this._db
        .executeSql('UPDATE myfamily SET email=? WHERE rowid=?', [
          this.data.email,
          rowid,
        ])
        .then((res) => {
    ...
    if (this.data.phone != '') {
      this._db
        .executeSql('UPDATE myfamily SET phone=? WHERE rowid=?', [
          this.data.phone,
          rowid,
        ])
        .then((res) => {
