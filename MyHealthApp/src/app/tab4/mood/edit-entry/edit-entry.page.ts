...
export class EditEntryPage {
  ...
  async updateSQL(rowid) {
    if (this.data.date != '') {
      this._db
        .executeSql('UPDATE mood SET date=? WHERE rowid=?', [
          this.data.date,
          rowid,
        ])
        .then((res) => {
    ...
    if (this.data.happy != '') {
      this._db
        .executeSql('UPDATE mood SET happy=? WHERE rowid=?', [
          this.data.happy,
          rowid,
        ])
        .then((res) => {
    ...
    if (this.data.independent != '') {
      this._db
        .executeSql('UPDATE mood SET independent=? WHERE rowid=?', [
    ...
    if (this.data.tired != '') {
      this._db
        .executeSql('UPDATE mood SET tired=? WHERE rowid=?', [
          this.data.tired,
          rowid,
        ])
        .then((res) => {
    ...
    if (this.data.productive != '') {
      this._db
        .executeSql('UPDATE mood SET productive=? WHERE rowid=?', [
    ...
    if (this.data.notes != '') {
      this._db
        .executeSql('UPDATE mood SET notes=? WHERE rowid=?', [
          this.data.notes,
          rowid,
        ])
        .then((res) => {
