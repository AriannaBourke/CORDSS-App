...
export class EditEntryPage {
  ...
  async updateSQL(rowid) {
    if (this.data.note_name != '') {
      this._db
        .executeSql('UPDATE thoughtsfeelings SET note_name=? WHERE rowid=?', [
          this.data.note_name,
          rowid,
        ])
        .then((res) => {
    ...
    if (this.data.notes != '') {
      this._db
        .executeSql('UPDATE thoughtsfeelings SET notes=? WHERE rowid=?', [
          this.data.notes,
          rowid,
        ])
        .then((res) => {
  ...
  deleteAll() {
    this._db
      .executeSql('DELETE FROM tfpictures WHERE cardid=?', [this.rowid])
      .then((res) => {
        this.getDataPictures(this.rowid);
      })
