...
export class EditEntryPage {
  ...
  public getData(rowid) {
    this._db
      .executeSql('SELECT * FROM basicinfo WHERE rowid=?', [rowid])
      .then((res) => {
        this.basicInfo = [];
  ...
  async updateSQL(rowid) {
    if (this.data.gender != '') {
      this._db
        .executeSql('UPDATE basicinfo SET gender=? WHERE rowid=?', [
          this.data.gender,
          rowid,
        ])
        .then((res) => {
    ...
    if (this.data.height != '') {
      this._db
        .executeSql('UPDATE basicinfo SET height=? WHERE rowid=?', [
          this.data.height,
          rowid,
        ])
        .then((res) => {
    ...
    if (this.data.weight != '') {
      this._db
        .executeSql('UPDATE basicinfo SET weight=? WHERE rowid=?', [
          this.data.weight,
          rowid,
        ])
        .then((res) => {
    ...
    if (this.data.blood_pressure != '') {
      this._db
        .executeSql('UPDATE basicinfo SET blood_pressure=? WHERE rowid=?', [
    ...
    if (this.data.allergies != '') {
      this._db
        .executeSql('UPDATE basicinfo SET allergies=? WHERE rowid=?', [
          this.data.allergies,
          rowid,
        ])
        .then((res) => {
    ...
    if (this.data.notes != '') {
      this._db
        .executeSql('UPDATE basicinfo SET notes=? WHERE rowid=?', [
          this.data.notes,
          rowid,
        ])
        .then((res) => {
