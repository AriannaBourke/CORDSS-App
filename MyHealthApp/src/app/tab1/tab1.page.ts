...
export class Tab1Page {
  ...
  default: any;
  AboutMeTable: string =
    'CREATE TABLE IF NOT EXISTS aboutme (rowid INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT, birthday TEXT, about TEXT, email TEXT, phone INT, address TEXT, nhs_number INT, emergency_1_name TEXT, emergency_1_number TEXT, emergency_2_name TEXT, emergency_2_number TEXT, emergency_3_name TEXT, emergency_3_number TEXT)';
  data = {
    name: '',
    birthday: '',
    about: '',
  ...
  AboutMeTablePicture: string =
    'CREATE TABLE IF NOT EXISTS aboutmepicture (rowid INTEGER PRIMARY KEY AUTOINCREMENT, picture TEXT)';
  datapicture = { picture: '' };
  ...
  verifyDatabasePopulated() {
    this._db.executeSql('SELECT * FROM aboutme', <any>[]).then((res) => {
  ...
  public getData() {
    ...
    this._db
      .executeSql('SELECT * FROM aboutme ORDER BY rowid DESC', <any>[])
      .then((res) => {
        this.aboutme = [];
  ...
  public getDataPicture() {
    this._db.executeSql('SELECT * FROM aboutmepicture', <any>[]).then((res) => {
      this.aboutmepicture = [];
      for (var i = 0; i < res.rows.length; i++) {
  ...
  public saveDataPicture() {
    this._db
      .executeSql('INSERT INTO aboutmepicture VALUES(NULL, ?)', [
  ...
  deleteData(rowid) {
    this._db
      .executeSql('DELETE FROM aboutme WHERE rowid=?', [rowid])
      .then((res) => {
        this.getData();
      })
