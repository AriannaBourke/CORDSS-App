...
export class UrgentplanPdfPage {
  ...
  UrgentPlanTable: string =
    'CREATE TABLE IF NOT EXISTS urgentplan (rowid INTEGER PRIMARY KEY AUTOINCREMENT, type TEXT, instructions TEXT, phone INT, notes TEXT)';
  ...
  downloadPdf() {
    if (this._plat.is('cordova')) {
      this.pdfObject.getBuffer((buffer) => {
        ...
        this.file
          .writeFile(this.file.dataDirectory, 'UrgentPlan.pdf', blob, {
            replace: true,
          })
          .then((fileEntry) => {
            this.fileOpener.open(
              this.file.dataDirectory + 'UrgentPlan.pdf',
              'application/pdf'
            );
          });
      });
